...
func (r *CourseRepository) List(ctx context.Context) ([]domain.Course, error) {
	rows, err := r.db.QueryContext(ctx, `SELECT id,name,target_language,reference_language,theory_language,created_at,updated_at FROM language_tracks ORDER BY id`)
	if err != nil {
...
func (r *CourseRepository) Get(ctx context.Context, id int64) (domain.Course, error) {
	var course domain.Course
	err := r.db.QueryRowContext(ctx, `SELECT id,name,target_language,reference_language,theory_language,created_at,updated_at FROM language_tracks WHERE id=?`, id).Scan(&course.ID, &course.Name, &course.TargetLanguage, &course.ReferenceLanguage, &course.TheoryLanguage, &course.CreatedAt, &course.UpdatedAt)
